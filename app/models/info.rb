class Info < ApplicationRecord
  ...
  # callbacks
  after_create_commit { self.log_work('upload_info') }
  ...
  def log_work(action = '', action_user_id = nil)
    trackable_url = url_helpers(only_path: true, controller: self.infoable.class.to_s.pluralize.downcase, action: 'show', id: self.infoable.id)
    worker_id = action_user_id || self.user_id
    Work.create!(trackable_type: "#{self.infoable.class.to_s}", trackable_id: self.infoable.id, trackable_url: trackable_url, action: "#{action}", user_id: worker_id, 
  ...
  def destroy_and_log_work(current_user_id)
    ...
    if destroyed_return
      destroyed_clone.log_work('destroy_info', current_user_id)
    end
    return destroyed_return
  end

  def send_notification_to_model
    infoable.send_notification_to_pool if (['Errand', 'Event', 'Project']).include? self.infoable.class.to_s     
  end
