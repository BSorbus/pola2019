class InspectionProtocol < ApplicationRecord
  delegate :url_helpers, to: 'Rails.application.routes'

  # relations
  belongs_to :user
  belongs_to :inspectionable, polymorphic: true

  # validates
  validates :attached_file, presence: true, file_size: { in: 1.byte..500.megabyte }

  # callbacks
  after_create_commit { self.log_work('upload_inspection_protocol') }
  ...
  def log_work(action = '', action_user_id = nil)
    trackable_url = url_helpers(only_path: true, controller: self.inspectionable.class.to_s.pluralize.downcase, action: 'show', id: self.inspectionable.id)
    worker_id = action_user_id || self.user_id
    Work.create!(trackable_type: "#{self.inspectionable.class.to_s}", trackable_id: self.inspectionable.id, trackable_url: trackable_url, action: "#{action}", user_id: worker_id, 
  ...
  def destroy_and_log_work(current_user_id)
    ...
    if destroyed_return
      destroyed_clone.log_work('destroy_inspection_protocol', current_user_id)
    end
    return destroyed_return
  end

  def send_notification_to_model
    inspectionable.send_notification_to_pool if (['Errand', 'Event', 'Project']).include? self.inspectionable.class.to_s
  end
