class Photo < ApplicationRecord
  ...
  # callbacks
  after_create_commit { self.log_work('upload_photo') }
  after_update_commit { self.log_work('update') }
  after_commit :send_notification_to_model


  mount_uploader :attached_file, PhotoUploader


  def log_work(action = '', action_user_id = nil)
    trackable_url = url_helpers(only_path: true, controller: self.photoable.class.to_s.pluralize.downcase, action: 'show', id: self.photoable.id)
    worker_id = action_user_id || self.user_id
    Work.create!(trackable_type: "#{self.photoable.class.to_s}", trackable_id: self.photoable.id, trackable_url: trackable_url, action: "#{action}", user_id: worker_id, 
  ...
  def destroy_and_log_work(current_user_id)
    ...
    if destroyed_return
      destroyed_clone.log_work('destroy_photo', current_user_id)
    end
    return destroyed_return
  end

  def send_notification_to_model
    photoable.send_notification_to_pool if (['Errand', 'Event', 'Project']).include? self.photoable.class.to_s     
  end
