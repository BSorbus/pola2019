class Statement < ApplicationRecord
  ...
  # relations
  belongs_to :user
  belongs_to :statemenable, polymorphic: true

  # validates
  validates :attached_file, presence: true, file_size: { in: 1.byte..500.megabyte }

  # callbacks
  after_create_commit { self.log_work('upload_statement') }
  ...
  def log_work(action = '', action_user_id = nil)
    trackable_url = url_helpers(only_path: true, controller: self.statemenable.class.to_s.pluralize.downcase, action: 'show', id: self.statemenable.id)
    worker_id = action_user_id || self.user_id
    Work.create!(trackable_type: "#{self.statemenable.class.to_s}", trackable_id: self.statemenable.id, trackable_url: trackable_url, action: "#{action}", user_id: worker_id, 
  ...
  def destroy_and_log_work(current_user_id)
    ...
    if destroyed_return
      destroyed_clone.log_work('destroy_statement', current_user_id)
    end
