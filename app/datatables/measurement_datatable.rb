class MeasurementDatatable < AjaxDatatablesRails::ActiveRecord
  extend Forwardable
  ...
  def get_raw_records
    if (options[:measurementable_id]).present? && (options[:measurementable_type]).present?
      Measurement.joins(:user).where(measurementable_id: options[:measurementable_id], measurementable_type: options[:measurementable_type]).all
    else
      Measurement.joins(:user).all
    end
  end
