class ThroughEventsInspectionProtocolDatatable < AjaxDatatablesRails::ActiveRecord
  extend Forwardable

  def_delegators :@view, :link_to, :truncate, :inspection_protocol_path, :show_through_events_inspection_protocol_path, :download_inspection_protocol_path, :edit_inspection_protocol_path, :t
  ...
  def view_columns
    @view_columns ||= {
      ...
      inspectionable: { source: "Event.title", cond: :like, searchable: true, orderable: true },
  ...
  def data
    records.map do |record|
      {
        id:             record.id,
        inspectionable:   record.inspectionable.title_as_link,
        attached_file:  link_to(truncate(record.attached_file_identifier, length: 150), download_inspection_protocol_path(record.id), title: t('tooltip.download'), rel: 'tooltip') + '  ' +  
                          link_to(' ', @view.show_through_events_inspection_protocol_path(record.id), remote: true, class: 'fa fa-eye pull-right', title: "Podgląd", rel: 'tooltip'),
  ...
  def get_raw_records
    if (options[:for_parent_id]).present? && (options[:for_parent_type]).present?
      if options[:for_parent_type] == 'Project'
        parent_inspectionable = Project.find(options[:for_parent_id])
        parent_inspectionable.events_inspection_protocols.joins(:user).all
      end
    end
  end
