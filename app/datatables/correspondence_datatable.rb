class CorrespondenceDatatable < AjaxDatatablesRails::ActiveRecord
  extend Forwardable
  ...
  def data
    records.map do |record|
      {
        id:             record.id,
        attached_file:  link_to(truncate(record.attached_file_identifier, length: 150), download_correspondence_path(record.id), title: t('tooltip.download'), rel: 'tooltip') + '  ' +  
  ...
  def get_raw_records
    if (options[:correspondenable_id]).present? && (options[:correspondenable_type]).present?
      Correspondence.joins(:user).where(correspondenable_id: options[:correspondenable_id], correspondenable_type: options[:correspondenable_type]).all
    else
      Correspondence.joins(:user).all
    end
  end
