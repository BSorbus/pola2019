class GroupUsersDatatable < AjaxDatatablesRails::ActiveRecord
  ...
  def link_add_remove(rec, has_group)
    if policy(:group).add_remove_group_user?
      if has_group
        "<div style='text-align: center'><button ajax-path='" + group_user_path(group_id: options[:only_for_current_group_id], id: rec.id) + "' ajax-method='DELETE' toastr-message='usunięto: " + rec.name + "' class='btn btn-xs btn-danger fa fa-minus'></button></div>"
      else
        "<div style='text-align: center'><button ajax-path='" + group_users_path(group_id: options[:only_for_current_group_id], id: rec.id) + "' ajax-method='POST' toastr-message='dodano: " + rec.name + "' class='btn btn-xs btn-success fa fa-plus'></button></div>"
      end
    else
      ""
    end
