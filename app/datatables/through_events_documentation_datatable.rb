class ThroughEventsDocumentationDatatable < AjaxDatatablesRails::ActiveRecord
  extend Forwardable
  ...
  def view_columns
    @view_columns ||= {
      ...
      documentationable:    { source: "Event.title", cond: :like, searchable: true, orderable: true },
    ...
    records.map do |record|
      {
        ...
        documentationable:    record.documentationable.title_as_link,
  ...
  def get_raw_records
    if (options[:for_parent_id]).present? && (options[:for_parent_type]).present?
      if options[:for_parent_type] == 'Project'
        parent_documentationable = Project.find(options[:for_parent_id])
        parent_documentationable.events_documentations.joins(:user).all
      end
    end
  end
