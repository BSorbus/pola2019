class OpinionDatatable < AjaxDatatablesRails::ActiveRecord
  extend Forwardable
  ...
  def data
    records.map do |record|
      {
        id:             record.id,
        attached_file:  link_to(truncate(record.attached_file_identifier, length: 150), download_opinion_path(record.id), title: t('tooltip.download'), rel: 'tooltip') + '  ' +  
  ...
  def get_raw_records
    if (options[:opinionable_id]).present? && (options[:opinionable_type]).present?
      Opinion.joins(:user).where(opinionable_id: options[:opinionable_id], opinionable_type: options[:opinionable_type]).all
    else
      Opinion.joins(:user).all
    end
  end
