class MeasurementPolicy < ApplicationPolicy
  ...
  def event_type_activities
    if @model.class.to_s == 'Symbol'
      ...
      case @model.measurementable.class.to_s
      when 'Customer'
        EventType.joins(events: {accessorizations: [:user], event_status: [], project: {customer: []}})
          .where(events: {accessorizations: {user_id: [@user]}, projects: {customer: [@model.measurementable]} })
          .merge(EventStatus.status_can_change)
          .select(:activities).distinct.map(&:activities).flatten
      when 'Project'
        EventType.joins(events: {accessorizations: [:user], event_status: [], project: []})
          .where(events: {accessorizations: {user_id: [@user]}, project: [@model.measurementable]})
          .merge(EventStatus.status_can_change)
          .select(:activities).distinct.map(&:activities).flatten
      when 'Errand'
        EventType.joins(events: {accessorizations: [:user], event_status: [], errand: []})
          .where(events: {accessorizations: {user_id: [@user]}, errand: [@model.measurementable]})
          .merge(EventStatus.status_can_change)
          .select(:activities).distinct.map(&:activities).flatten
      when 'Event'
        EventType.joins(events: {accessorizations: [:user], event_status: []})
          .where(events: {accessorizations: {user_id: [@user], event_id: [@model.measurementable]}})
          .merge(EventStatus.status_can_change)
          .select(:activities).distinct.map(&:activities).flatten
      end
    end      
    ...
    if @model.class.to_s == 'Symbol'
      ...
      when 'Customer'
        Role.joins(accessorizations: {user:[], event: {event_status:[], project: [:customer]}})
          .where(accessorizations: {user: [@user], events: {projects: {customer: [@model.measurementable]}}})
          .merge(EventStatus.status_can_change)
          .select(:activities).distinct.map(&:activities).flatten
      when 'Project'
        Role.joins(accessorizations: {user:[], event: [:event_status, :project]})
          .where(accessorizations: {user: [@user], events: {project: [@model.measurementable]}})
          .merge(EventStatus.status_can_change)
          .select(:activities).distinct.map(&:activities).flatten
      when 'Errand'
        Role.joins(accessorizations: {user:[], event: [:event_status, :errand]})
          .where(accessorizations: {user: [@user], events: {errand: [@model.measurementable]}})
          .merge(EventStatus.status_can_change)
          .select(:activities).distinct.map(&:activities).flatten
      when 'Event'
        Role.joins(accessorizations: {user:[], event: [:event_status]})
          .where(accessorizations: {user: [@user], event: [@model.measurementable]})
          .merge(EventStatus.status_can_change)
          .select(:activities).distinct.map(&:activities).flatten
      end
    end
