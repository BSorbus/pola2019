...
#    attachment_parent_filter = params[:attachment_parent_id].present? ? params[:attachment_parent_id] : nil
  ...
  def move_to_original_documentation
    @attachment = Attachment.find(params[:id])
    attachment_authorize(@attachment, "destroy", @attachment.attachmenable_type.singularize.downcase)
    if @attachment.move_to_original_documentation_and_log_work(current_user.id)
      head :no_content
    end 
  end

  def move_to_final_documentation
    @attachment = Attachment.find(params[:id])
    attachment_authorize(@attachment, "destroy", @attachment.attachmenable_type.singularize.downcase)
    if @attachment.move_to_final_documentation_and_log_work(current_user.id)
      head :no_content
    end 
  end
